...
*   🎨 **Contraste Sincronizado**: El `colorDiffThreshold` del MotionEngine del Lab ahora usa el valor real del slider de contraste del dashboard (`105 - contrast`).
...
*   📜 **Logger**: Nueva traza web para cambios en el estado del Pre-Record Buffer.
...
## [v3.9.10-dev.30] - 2026-02-07
### Changed
- **Web Motion Lab**: Portada la lógica exacta de `MotionDetector.java` a JavaScript (`MotionEngine`). Ahora incluye `STRIDE=10`, `THRESHOLD=50` y el **Smart Light Filter** (ratio > 0.60).

## [v3.9.10-dev.29] - 2026-02-07
### Fixed
- **Web Motion Lab**: Corregido `SyntaxError` crítico causado por código residual al mover la lógica de inicialización.

## [v3.9.10-dev.28] - 2026-02-07
### Fixed
- **Web Motion Lab**: Corregido error de *hoisting* en JS moviendo la inicialización al final del script para evitar `ReferenceError` con la clase `MJPEGPlayer`.
...
## [v3.9.10-dev.13] - 2026-02-07
### Fixed
- **Compilation**: Corregido error de sintaxis en `NanoHttpServer` (block duplicate) que impedía la compilación.

## [v3.9.10-dev.12] - 2026-02-07
### Added
- **Smart Light Filter**: Algoritmo anti-falsos positivos que ignora cambios de iluminación global masivos (>60% de la imagen).
- **Web UI**: Toggle "Filtro Anti-Luces" en el menú de Ajustes para control en tiempo real.
...
## [v3.9.10-dev.5] - 2026-02-06
### Added
- **UI Security**: El campo "Telegram Bot Token" ahora está oculto por defecto (password type) con botón de revelar.
...
- **Logging**: Agregado logging de errores de red en la alerta (antes fallaba silenciosamente).
...
## [v3.9.9-dev.11] - 2026-02-03
### Added
- **Telegram Autoplay**: Transcodificación automática de Preview (MJPEG -> MP4) antes de subir.
- **Resilience**: Fallback automático a MJPEG (.avi) si la transcodificación falla.

## [v3.9.9-dev.10] - 2026-02-03
### Changed
- **UX**: (Telegram) Cambiada extensión de subida de .mjpeg a .avi en tiempo real para habilitar "Compartir con VLC" en iOS (Share Sheet Compatibility).
...
- **Security**: Ahora `HttpsURLConnection` usa el Provider por defecto (Conscrypt) sin interferencias manuales.
- **Stability**: Implementada captura de errores detallada (`getErrorStream`) para diagnósticos 4xx/5xx de Telegram.
...
## [v3.9.7-dev.42] - 2026-01-30
### Fixed
- **Lazy Rotation**: La imagen ya NO se rota incondicionalmente al recibir el frame.
...
- **v3.9.10-dev.64 (Anti-Zombie)**: Solucionado bug en  donde una variable estática mantenía vivas conexiones muertas, causando sobrecalentamiento en reposo. Implementado .
